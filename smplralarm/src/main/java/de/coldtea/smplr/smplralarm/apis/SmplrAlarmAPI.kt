...
class SmplrAlarmAPI(val context: Context) {
    ...
    fun isVibrate(isActive: () -> Boolean) {
        this.isVibrate = isActive()
    }
    ...
    fun nameAlarm(name: () -> String) {
        this.nameAlarm = name()
    }

    fun setVoicePath(voicePath: () -> String) {
    ...
    internal fun updateAlarm() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                ...
                val updateNameInfo =
                    if (nameAlarm == null) alarmNotification.nameAlarm else nameAlarm
